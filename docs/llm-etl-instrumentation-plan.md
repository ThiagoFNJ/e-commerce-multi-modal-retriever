...
| G4 | Output determinism unmeasured | **Determinism probe**: ~1k length-stratified fixed sample, N=3 identical-config runs *during* production load (realistic co-batching) + N≥1 after. Metrics: exact-string match, parse rate, schema conformance, per-record Jaccard distribution, drift taxonomy. Hypothesis: continuous-batching numerics break temp-0 determinism. | during + post | executing |
| G5 | Cost counterfactual missing | Price G1's token counts against 2–3 hosted APIs; publish cost/1k reviews self-hosted (spot, incl. idle/incident hours honestly) vs API | post-run | planned |
